...
pet1 = Pet.create!(name: "Happy", birthday: "15-06-2015", race: "Border Collie", gender: "femelle", shelter_id: first_shelter.id, category: "chien", description: "Happy aime être dehors dans les grands espaces ! Elle adore les balades et rêve de trouver une famille", family_friendly: true, cat_friendly: true, main_quality:"Sportif", is_adopted: false)
...
pet3 = Pet.create!(name: "Junior", birthday: "06-01-2015", race: "Jack Russel", gender: "mâle", shelter_id: second_shelter.id, category: "chien", description: "Junior est un adorable Jack Russel très câlin et énergique.
  Il pourra vivre en compagnie de chats.", main_quality:"Affectueux", family_friendly: true, cat_friendly: true, is_adopted: false)
...
pet4 = Pet.create!(name: "Django", birthday: "26-03-2013", race: "Berger allemand", gender: "mâle", shelter_id: second_shelter.id, category: "chien", description: "Django est un chien câlin une fois en confiance. Il est sociable avec ses congénères femelles. En revanche il n’apprécie pas les chats. Django peut se montrer agressif lorsqu’il stresse ou chez le vétérinaire. Des maîtres expérimentés seront demandés.", main_quality:"Joueur", family_friendly: false, cat_friendly: false, is_adopted: false)
...
pet5 = Pet.create!(name: "Oslo", birthday: "22-11-2019", race: "Husky", gender: "mâle", shelter_id: first_shelter.id, category: "chien", description: "Oslo est un jeune chien exceptionnellement intelligent mais qui par conséquent demandera énormément de stimulations
  il a un très grand potentiel pour la recherche olfactive, il ne pourra pas partager son foyer avec un chat", main_quality:"Affectueux", family_friendly: true, cat_friendly: false, is_adopted: false)
...
pet7 = Pet.create!(name: "Lola", birthday: "21-06-2018", race: "Bichon", gender: "femelle", shelter_id: fourth_shelter.id, category: "chien", description: "Lola est une chienne adorable, qui adore les câlins et les cours d'agility", main_quality:"Espiègle", family_friendly: true, cat_friendly: true, is_adopted: false)
...
pet8 = Pet.create!(name: "Bella", birthday: "01-01-2020", race: "Bouvier Bernois", gender: "femelle", shelter_id: fourth_shelter.id, category: "chien", description: "Trouvée dans la rue, Bella est adorable, elle est également sociable avec les autres chiens et semble facile à vivre.", main_quality:"Doux avec les enfants", family_friendly: true, cat_friendly: true, is_adopted: false)
...
pet10 = Pet.create!(name: "Rocky", birthday: "02-12-2016", race: "Bouledogue", gender: "mâle", shelter_id: third_shelter.id, category: "chien", description: "Rocky vivait avec deux grands enfants, un petit chien et un chat. Pourtant, arrivé au refuge, le contact avec ses congénères l’a beaucoup stressé.
  Aujourd'hui, il n'accepte plus ni les chats, ni les chiens. Il est même devenu très réactif. Rocky aime les câlins mais peut se montrer brut s’il monte en excitation.", main_quality:"Joueur", family_friendly: false, cat_friendly: false, is_adopted: false)
...
pet1 = Pet.create!(name: "Flocon", birthday: "06-06-2018", race: "Européen", gender: "mâle", shelter_id: fifth_shelter.id, category: "chat", description: "C'est un chat très craintif qui aura besoin de beaucoup de patience et de douceur. Il est positif au sida du chat. Il devra donc vivre en appartement ou en maison mais sans accès extérieur.", main_quality:"Espiègle", family_friendly: true, cat_friendly: false, is_adopted: false)
...
pet2 = Pet.create!(name: "Karma", birthday: "09-09-2019", race: "Européen", gender: "femelle", shelter_id: fifth_shelter.id, category: "chat", description: "Karma est une belle demoiselle assez timide envers les humains.
  Un travail de socialisation sera à continuer dans sa futur famille.
  La présence de jeunes enfants n'est pas conseillé dans son nouvel environnement.", main_quality:"Affectueux", family_friendly: true, cat_friendly: true, is_adopted: false)
...
pet3 = Pet.create!(name: "Titeuf", birthday: "01-01-2008", race: "Européen", gender: "mâle", shelter_id: fifth_shelter.id, category: "chat", description: "Titeuf est une magnifique minette indépendante. Elle aura besoin de temps pour faire confiance.", main_quality:"Espiègle", family_friendly: true, cat_friendly:true, is_adopted: false)
...
pet4 = Pet.create!(name: "Sacha", birthday: "01-10-2014", race: "Européen", gender: "mâle", shelter_id: fourth_shelter.id, category: "chat", description: "C'est un chat très craintif qui aura besoin de beaucoup de patience et de douceur. Il est positif au sida du chat. Il devra donc vivre en appartement ou en maison mais sans accès extérieur.", main_quality:"Curieux", family_friendly: false, cat_friendly: false, is_adopted: false)
...
pet5 = Pet.create!(name: "Tisha", birthday: "05-06-2022", race: "Européen", gender: "femelle", shelter_id: fifth_shelter.id, category: "chat", description: "C'est un chat sociable qui s'adaptera facilement à son nouveau foyer. Un extérieur serait un plus pour lui.", main_quality:"Joueur", family_friendly: true, cat_friendly: true, is_adopted: false)
...
pet6 = Pet.create!(name: "Papaya", birthday: "01-11-2020", race: "Européen", gender: "femelle", shelter_id: first_shelter.id, category: "chat", description: "Papaya est craintive et ne vient pas au contact. Avec de la patience , elle pourra améliorer son comportement ; Elle est trés mignonne et vient de la rue.", main_quality:"Joueur", family_friendly: false, cat_friendly: true, is_adopted: false)
...
pet7 = Pet.create!(name: "Yoshi", birthday: "01-06-2022", race: "Européen", gender: "mâle", shelter_id: first_shelter.id, category: "chat", description: "C'est un chat sociable qui s'adaptera facilement à son nouveau foyer. Un extérieur serait un plus pour lui.", main_quality:"Doux avec les enfants", family_friendly: true, cat_friendly:true, is_adopted: false)
...
pet8 = Pet.create!(name: "Musti", birthday: "01-12-2021", race: "Européen", gender: "femelle", shelter_id: second_shelter.id, category: "chat", description: "C'est un chat sociable qui s'adaptera facilement à son nouveau foyer.", main_quality:"Joueur",  family_friendly: true, cat_friendly:true, is_adopted: false)
...
pet9 = Pet.create!(name: "Diablo", birthday: "23-12-2015", race: "Européen", gender: "mâle", shelter_id: second_shelter.id, category: "chat", description: "Diablo est un gentil loulou de caractère. Il aime sa tranquillité et n'hésite pas à se faire comprendre lorsqu'il en a assez.", main_quality:"Affectueux", family_friendly: false, cat_friendly:true, is_adopted: false)
...
pet10 = Pet.create!(name: "Rouka", birthday: "25-07-2022", race: "Européen", gender: "mâle", shelter_id: third_shelter.id, category: "chat", description: "C'est un chat sociable qui s'adaptera facilement à son nouveau foyer.", main_quality:"Affectueux", family_friendly: true, cat_friendly: true, is_adopted: false)
...
puts "#{ListPet.count} insert of pets in wishlists created"
